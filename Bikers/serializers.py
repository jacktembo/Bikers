...
class PackageSerializer(serializers.ModelSerializer):
    ...
    total_price = serializers.SerializerMethodField(method_name='the_total_price')
    transit_status = serializers.SerializerMethodField(method_name='the_transit_status')
    ...
    def the_transit_status(self, package: Package):
        return 'true' if datetime.now().replace(tzinfo=utc) > (datetime(package.departure_date.year, package.departure_date.month, package.departure_date.day) + timedelta(hours=package.departure_time.hour)).replace(tzinfo=utc) else 'false'
    def the_total_price(self, package: Package):
        insurance_percentage = PackageInsurance.objects.first().percentage
        if package.insurance:
            return package.price + (package.package_value * insurance_percentage / 100)
    ...
    class Meta:
        ...
        fields = [
            ...
            'starting_town_coordinates', 'delivery_town_coordinates', 'insurance',
            'description', 'ticket_number', 'total_price',
        ]
